        ...
        void PathMove(std::vector<Point> path, T goal_pt_track_time = 100, T maxSpeed = 100, T maxtime_ms = 10000, T move_gaptime = 10 ,int fwd = 1, int back = 0){
            ...
            while(cnt <= 15){
                ...
                if(LastFoundIndex == path.size() - 1){       // 最后一段直接直线移动到目标点即可
                    ...
                    // 最后一段直接直线移动到目标点
                    if(targetDeg <= 100 || fabs(targetDeg - 360) <= 100){
                    ...
                    if(fabs(GetDistance(position->globalPoint, path[path.size() - 1])) <= 5 * errorThredhold){
                        ...
                        if(targetDeg <= 100 || fabs(targetDeg - 360) <= 100){
                            ...
                            moveToTarget(goal_point, maxSpeed, 1000, move_gaptime, fwd);
                        }
                        break;
                    }else{
                        // 最后一段直接直线移动到目标点
                        if(targetDeg <= 100 || fabs(targetDeg - 360) <= 100){
                            RotMoveToTarget(goal_point, maxSpeed , goal_pt_track_time, move_gaptime, fwd);
                        }else{
                            moveToTarget(goal_point, maxSpeed, goal_pt_track_time, move_gaptime, fwd);
                        }
                    }

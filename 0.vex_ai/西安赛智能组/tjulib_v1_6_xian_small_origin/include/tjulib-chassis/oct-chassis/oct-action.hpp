    ...
    public:
        ...
        void MoveForRing(Point target, T goal_pt_track_time = 100, T maxSpeed = 100, T maxtime_ms = 15000, T move_gaptime = 10 ,int fwd = 1, int back = 0){
            ...
            if(fabs(GetDistance(position->globalPoint, target)) >= radius){  
                ...
                if(GetDistance(position->globalPoint, target) > 65){
                    ...
                    /*= Step1.2 : RRT规划器获取轨迹 =*/
                    printf("rrt2_start\n");   
                    path = _rrt_long.rrt_planning({position->globalPoint.x, position->globalPoint.y, 0}, (Point)(target_pt));  
                     for(auto point : path){
                         printf("{x:%lf , y:%lf}\n", point.x, point.y);
                     }
                    printf("x0 : %lf, y0 : %lf \n", gps_x, gps_y);
                    ...
                    /*= Step1.2 : RRT规划器获取轨迹 =*/
                    printf("rrt2_start\n");   
                    path = _rrt_short.rrt_planning({position->globalPoint.x, position->globalPoint.y, 0}, (Point)(target_pt));  
                     for(auto point : path){
                         printf("{x:%lf , y:%lf}\n", point.x, point.y);
                     }
                    printf("x0 : %lf, y0 : %lf \n", gps_x, gps_y);
        ...
        void MoveForRingHalf(Point target, T goal_pt_track_time = 100, T maxSpeed = 100, T maxtime_ms = 15000, T move_gaptime = 10 ,int fwd = 1, int back = 0){
            ...
            if(fabs(GetDistance(position->globalPoint, target)) >= radius){  
                ...
                /*= Step1.3 : 规划路径跟踪 =*/
                PathMove(path, goal_pt_track_time, 65, maxtime_ms, move_gaptime, fwd, back);
            ...
            reinforce_stop = false;
            half_ring_get = true;
            turnToTarget(target,  maxSpeed * 0.85, 1100,  fwd, back);
            setStop(hold);
            moveToTarget(target, 75, 800, move_gaptime, fwd);
            setStop(hold);
            simpleMove(30, 0, 0.10, 10);
           // setStop(hold);
            // 时间用于套环
            this_thread::sleep_for(600);
        }
        ...
        void MoveForStake(Point target, T goal_pt_track_time = 100, T maxSpeed = 100, T maxtime_ms = 15000, T move_gaptime = 10 ,int fwd = 1, int back = 1){
           ...
           /*= Step1 : 如果机器人在操作半径外则需要先到达可执行的操作半径范围内 =*/
            ...
            if(fabs(GetDistance(position->globalPoint, target)) >= radius){  
                std::vector<Point> path;
                if(GetDistance(position->globalPoint, target) > 80){
                    /*= Step1.1 : 计算轨迹与圆的交点 =*/
                     Point target_pt = CalcuTargetPoint(_rrt_long, target, radius);
                     target_pt.angle = 0;
                    /*= Step1.2 : RRT规划器获取轨迹 =*/
                ...
                /*= Step1.3 : 规划路径跟踪 =*/
                PathMove(path, goal_pt_track_time, 50, maxtime_ms, move_gaptime, fwd, back);
            }
            
            /*= Step2 :执行最后一步动作 =*/
            turnToTarget(target,  50, 1100,  fwd, back);
            moveToTarget(target, 45, 1200, move_gaptime, fwd);
        ...
        void LayDownStake(Point target, T goal_pt_track_time = 100, T maxSpeed = 100, T maxtime_ms = 15000, T move_gaptime = 10 ,int fwd = 1, int back = 1){
            ...
            if(fabs(GetDistance(position->globalPoint, target)) >= radius){  
                ...
                if(target.x == target_ && target.y == target_){
                    ...
                    task::sleep(300);
                    moveToTarget({step_back, step_back}, maxSpeed, 800, move_gaptime, fwd);
                    ...
                    if(target.x == target_ && target.y == target_){
                        simpleMove(50, 180, 0.65, 10);
                        turnToAngle(42, maxSpeed, maxtime_ms, fwd, back);
                        simpleMove(45, 180, 0.5, 10);
                        gas_hold.state(0, pct);
                        reinforce_stop = true;
                        task::sleep(500);
                        moveToTarget({step_back, step_back}, maxSpeed*0.6, maxtime_ms, move_gaptime, fwd);
                    }else if(target.x == -target_ && target.y == target_){
                        simpleMove(50, 180, 0.5, 10);
                        turnToAngle(150, maxSpeed * 0.6, 1200, fwd);
                        simpleMove(60, 180, 0.4, 10);
                        gas_hold.state(0, pct);
                        reinforce_stop = true;
                        task::sleep(300);
                        moveToTarget({-step_back, step_back}, maxSpeed*0.75, maxtime_ms, move_gaptime, fwd);
                    }else if(target.x == target_ && target.y == -target_){
                        simpleMove(50, 180, 0.5, 10);
                        turnToAngle(139, maxSpeed, maxtime_ms, fwd, back);
                        simpleMove(50, 180, 0.65, 10);
                        gas_hold.state(0, pct);
                        manual = true;
                        reinforce_stop = true;
                        ...
                        turnToAngle(43, maxSpeed, maxtime_ms, fwd);
                        simpleMove(100, 180, 0.5, 10);
                        gas_hold.state(0, pct);
                        manual = true;
                        reinforce_stop = true;
                        task::sleep(500);
        ...
        void SlamDownRing(Point target, T goal_pt_track_time = 100, T maxSpeed = 100, T maxtime_ms = 15000, T move_gaptime = 10 ,int fwd = 1, int back = 1){
            ...
            if(target.x == 0 && target.y == target_){
                ...
                moveToTarget(target_pt2, maxSpeed * 0.7, maxtime_ms, move_gaptime);
                task::sleep(300);
                // 先正向对齐
                turnToAngle(0, maxSpeed, maxtime_ms);
                setStop(hold);
                // 对齐侧向
                VisionSensorMove(154, maxSpeed * 0.7, maxtime_ms, 0);
                task::sleep(300);
                // 转向桩
                turnToAngle(180, maxSpeed, maxtime_ms);
                // 对齐正向
                DistanceSensorMove(220, maxSpeed * 0.3, 5000, 0);
                setStop(hold);
                // 正向对齐固定桩
                task::sleep(300);
                // 转向桩

...
class HungarianAssigner3D(BaseAssigner):
    ...
    def assign(self,
        ...
        bboxes3d = denormalize_bbox(bbox_pred, self.pc_range) 
        iou3d = bbox_overlaps_nearest_3d(bboxes3d, gt_bboxes, coordinate='depth')
        
        cls_cost = self.cls_cost(cls_pred, gt_labels)
        #cls_cost = self.cls_cost(cls_pred, gt_labels, iou3d)

        # regression L1 cost
        reg_cost = self.reg_cost(bbox_pred[:, :8], normalized_gt_bboxes[:, :8])
        iou_cost = self.iou_cost(bboxes3d, gt_bboxes)
        ...
        nq = num_query
        ng = int(cost.shape[0] // nq)
        matched_row_inds, matched_col_inds = [], []
        for g in range(ng):
            # matched_row_inds1, matched_col_inds1 = linear_sum_assignment(cost[g*nq:(g+1)*nq])
            matched_row_inds1, matched_col_inds1 = linear_sum_assignment(cost[g*nq:(g+1)*nq].repeat(1, gt_repeattimes) )
            matched_row_inds.append(g*nq + matched_row_inds1)
            #matched_col_inds.append(matched_col_inds1)
            matched_col_inds.append(matched_col_inds1 % cost.shape[1])

...
class RandomAffine(object):
    ...
    def __call__(self, img):
        ...
        """
        ret = self.get_params(self.degrees, self.translate, self.scale, self.shear, img[0].size)
        out_imgs = []
        for Img in img[0:]:
            out_imgs.append(F.affine(Img, *ret, resample=self.resample, fillcolor=self.fillcolor))
        return out_imgs
...
class AddGaussianNoise(object):
    ...
    def __call__(self, imgs):
        if random.random() > 0.7:
            ...
            imgs = Image.fromarray(img.astype('uint8')).convert('RGB')

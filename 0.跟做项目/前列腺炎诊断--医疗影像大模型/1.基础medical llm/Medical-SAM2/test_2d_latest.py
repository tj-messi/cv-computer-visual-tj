...
def split_filename_to_array(filename):
    """
    将文件名通过 "+" 分割，并将分割结果存入数组中。
    输入: xxxxx1+xxxxx2+xxxxx3+xxxxx4
    输出: ['xxxxx1', 'xxxxx2', 'xxxxx3', 'xxxxx4']
    """
    # 使用字符串的 split 方法按 "+" 分割
...
def vis_image_for_input(imgs, pred_masks, gt_masks, save_path, reverse = False, points = None):
    ...
    if c == 2: # for REFUGE multi mask output
        ...
        for i in range(row_num):
            ...
            # 提取原图像的名字（假设 save_path 是通用路径）
            original_file_name = os.path.basename(save_path)  # 获取保存路径中的文件名
            base_name, _ = os.path.splitext(original_file_name)  # 分离文件名和扩展名

            seperate_name_list = split_filename_to_array(base_name)

            # 生成唯一文件名：原图像名+_segment_i.png
            single_save_path = os.path.join(os.path.dirname(save_path), f"{seperate_name_list[i]}_segment.png")
            
            if(single_save_path.find("Test")):
                single_save_path =  single_save_path.replace("Test", "")

...
def diagnois(request):
    ...
    if request.session.get('file_uploaded', False):
        ...
        '''
         # 读取文本结果
        result_text_path = Path('llm/static/result/result_text.txt')
        if result_text_path.exists():
            with open(result_text_path, 'r') as file:
                result_text = file.read().strip()
        else:
            result_text = "未找到诊断结果文本。"

        # 图表的路径，使用 static
        result_chart_path = static('llm/static/result/result_chart.jpg')  # 确保这里定义了路径
        ...
        message = "暂无文件信息。"
        result_chart_path = None  # 如果没有上传状态，确保 result_chart_path 有默认值

    return render(request, 'diagnois.html', {'message': message, 'result_chart': result_chart_path})

...
def extract_on_paths(file_paths):
	...
	for res_path, gt_path in file_paths:
		...
		if True:
			...
			# print("input_im size,format,mode:",input_im.size,input_im.format,input_im.mode)
			input_im, _ = mtcnn.align(input_im)
			if input_im is None:
				print('{} skipping {}'.format(pid, res_path))
				continue
			# print("input_im",id_transform(input_im).shape)
			input_id = facenet(id_transform(input_im).unsqueeze(0).cuda())[0]

			result_im = Image.open(gt_path)
			result_im = result_im.convert('RGB')
			# print("result_im size,format,mode:",result_im.size,result_im.format,result_im.mode)
			...
			# print("result_im",id_transform(result_im).shape)
			score = float(input_id.dot(result_id))
			print(res_path,"score:",score)

...
def validate(val_loader, model, criterion, epoch, device):
    ...
    with torch.no_grad():
        for i, (file_name, images,videos, questions, labels) in enumerate(val_loader, 0):
            # label
            labels = labels.to(device)
        
            # model forward pass
            outputs = model(image=images.to(device),video = videos.to(device) ,question=questions)

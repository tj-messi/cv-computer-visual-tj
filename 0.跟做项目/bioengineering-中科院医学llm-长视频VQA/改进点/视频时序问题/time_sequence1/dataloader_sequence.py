...
class Pit24VQAClassification(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        frames_num = get_95_percent("/home/test/PitVQA-main/time_sequence/text_len.txt")
        video = []
        video.append(img)
        ...
        while (1) :
            ...
            if compare_files(selected_txt,right_txt_path) :
                ...
                video.append(right_frame)
            else:
                break
            step+=1
        # 最后根据95分位进行裁剪和填充
        video = padding_and_slicing(video,frames_num)
        print("img_location: ",img_loc," video-length: ",len(video))
        '''
        # 2025-2-12 zjz
        ...
        return img_loc, img, video, question, label

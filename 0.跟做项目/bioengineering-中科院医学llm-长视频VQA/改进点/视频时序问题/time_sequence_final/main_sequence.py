...
def train(train_dataloader, model, criterion, optimizer, epoch, device):
    ...
    for i, (_, images, videos ,questions, labels) in  enumerate(tqdm(train_dataloader, desc=f"Epoch {epoch}", unit="batch")):
        # labels
...
def validate(val_loader, model, criterion, epoch, device):
    ...
    with torch.no_grad():
        for i, (file_name, images,videos, questions, labels) in enumerate(tqdm(train_dataloader, desc="val", unit="batch")):
            # label
            labels = labels.to(device)
...
if __name__ == '__main__':
    ...
    if args.dataset == 'endo18':
        ...
        folder_head = '/data/coding/PitVQA-main/PitVQA_dataset/qa-classification/video_'
        folder_tail = '/*.txt'

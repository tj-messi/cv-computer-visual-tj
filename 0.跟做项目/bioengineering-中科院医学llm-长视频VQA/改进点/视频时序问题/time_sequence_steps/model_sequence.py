...
from transformers import GPT2Tokenizer, GPT2Model, ViTModel, VivitModel, VivitConfig ,VivitImageProcessor
...
class VideoFeatureExtractor(nn.Module):
    def __init__(self):
        ...
        config = VivitConfig.from_pretrained("/data/coding/PitVQA-main/local_VIVIT")
        config.num_frames = 4  # 将帧数设置为 4
        self.vivit_model = VivitModel(config)

        
    def forward(self, video_frames):
        # video_frames : torch.Size([16, 16, 3, 224, 224])
        self.vivit_model.config.num_frames = video_frames.size(1)
...
class PitVQANet(nn.Module):
    def __init__(self, num_class=18):  # 18/59
        ...
        # decoder
        self.gpt_decoder = GPT2Model.from_pretrained('/data/coding/PitVQA-main/local_gpt2')
    ...
    def forward(self, image, video, question):
        ...
        # videos = torch.Size([16, 16, 3, 224, 224])

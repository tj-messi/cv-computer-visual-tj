...
import os
import sys
sys.path.append('/home/hanyu/JYX/new/TMI12.8/')
...
#         if deterministic:
#             loader = DataLoader(
#                 self.data, batch_size=self.batch_size, shuffle=False, num_workers=16, drop_last=True, pin_memory=True,prefetch_factor=4, persistent_workers=True,
#             )
#         else:
#             loader = DataLoader(
#                 self.data, batch_size=self.batch_size, shuffle=True, num_workers=16, drop_last=True, pin_memory=True,prefetch_factor=4, persistent_workers=True,
#             )
...
#         # SR_ST_all[index], self.spot_ST_all[index], self.WSI_5120_all[index], self.WSI_320_all[index]
#         for SR_ST, spot_ST, WSI_5120, Gene_index_map in loader:#new1.5: gene_class, Gene_index_map,WSI_320, gene_class
#             model_kwargs = {"low_res": spot_ST, "WSI_5120": WSI_5120, "Gene_index_map": Gene_index_map}##new1.5: gene_class, Gene_index_map
...
#             avg_time_per_step = total_duration / (self.step + 1)  # +1 避免除零
...
class TrainLoop:
    ...
    def get_dataloader(self):
        loader = self.load_dataloader()
        # SR_ST_all[index], self.spot_ST_all[index], self.WSI_5120_all[index], self.WSI_320_all[index]
        for SR_ST, spot_ST, WSI_5120,WSI_320, gene_class, Gene_index_map,gene_name_features,metadata_feature in loader:#new1.5: gene_class, Gene_index_map
            model_kwargs = {"low_res": spot_ST, "WSI_5120": WSI_5120, "WSI_320": WSI_320,"gene_class": gene_class, "Gene_index_map": Gene_index_map,'gene_name_features':gene_name_features,'metadata_feature':metadata_feature }##new1.5: gene_class, Gene_index_map
            yield SR_ST, model_kwargs
    ...
    def _load_parameters(self):
        ...
        if resume_checkpoint:
            ...
            self.model.load_state_dict(
                th.load(
                    resume_checkpoint, map_location=th.device("cuda" if th.cuda.is_available() else "cpu")
                )
            )
    ...
    def _load_optimizer_state(self):
        ...
        if bf.exists(opt_checkpoint):
            logger.log(f"loading optimizer state from checkpoint: {opt_checkpoint}")
            state_dict = th.load(
                opt_checkpoint, map_location=th.device("cuda" if th.cuda.is_available() else "cpu")
            )
            self.opt.load_state_dict(state_dict)

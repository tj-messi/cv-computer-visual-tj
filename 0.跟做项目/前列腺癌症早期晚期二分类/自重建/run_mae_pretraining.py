...
def get_args():
    ...
    parser.add_argument(
        '--decoder_depth', default=10, type=int, help='depth of decoder')
...
def main(args):
    ...
    MODEL_PATH = '/media/tongji/VideoMAEv2-master/pretrain_model/checkpoint_k400.pth'
    ...
    # 3. 创建新的状态字典，映射预训练权重到当前模型
    print("loaded_model_key&name")
    new_state_dict = {}
    for key, value in model_state_dict.items():
        print(key)
        # 去除预训练模型中的 'module.' 前缀（如果存在）
        new_key = key.replace('module.', '')
        
        mapped_key = '_orig_mod.' + new_key
        if mapped_key in model.state_dict():
                new_state_dict[mapped_key] = value
                print(f"Mapping {new_key} -> {mapped_key}")
        else:
                print(f"Skipping {new_key}: not found in current model")
        
        # 映射 head 相关的参数
        # elif new_key in ['head.weight', 'head.bias']:
        #     # 将预训练模型的 head 参数映射到当前模型的 _orig_mod.decoder.head
        #     mapped_key = '_orig_mod.decoder.' + new_key
        #     if mapped_key in model.state_dict():
        #         new_state_dict[mapped_key] = value
        #         print(f"Mapping {new_key} -> {mapped_key}")
        #     else:

...
def load_data(data_root,dataset_use,status,SR_times,gene_num,all_gene,gene_order=None,gene_name_order=None
    ...
    elif 'VisiumHD_moueembryo_sorted_data1' in dataset_use:
    ...
    elif 'Xenium_BreastCancer' in dataset_use:
       dataset= Xenium_humanbreast(data_root,dataset_use,SR_times,status,gene_num,all_gene,gene_order,gene_name_order)
    return dataset
...
class Xenium_humanbreast(Dataset):
    def __init__(self, data_root, dataset_use, SR_times, status, gene_num, all_gene, gene_order, gene_name_order):
        ...
        for sample_id in sample_name:
            base_path = os.path.join(data_root, dataset_use, "HR_ST", "extract", sample_id)
            print("Scanning:", base_path)
            if not os.path.exists(base_path):
                print(f"⚠️ Sample path not found: {base_path}")
                continue
            sub_patches = os.listdir(base_path)
            for patch_id in sub_patches:
                if len(patch_id.split('_')) < 2:
                    continue
        ...
        for sample_id, patch_id in self.selected_patches:
            # 加载 HR ST
            hr_path = os.path.join(data_root, dataset_use, "HR_ST", "extract", sample_id, patch_id,
                                   f'HR_ST_256.npz' if SR_times == 10 else 'HR_ST_128.npz')
            SR_ST = sp_sparse.load_npz(hr_path).toarray().reshape(
                256 if SR_times == 10 else 128,
                256 if SR_times == 10 else 128,
                280
            )
            SR_ST = np.transpose(SR_ST, (2, 0, 1))
            SR_ST_all.append(SR_ST)
            self.gene_scale.append(gene_order)

            # 加载 spot ST
            spot_path = os.path.join(data_root, dataset_use, "spot_ST", "extract", sample_id, patch_id, "spot_ST.npy")
            spot_ST = np.load(spot_path)
            spot_ST = np.transpose(spot_ST, (2, 0, 1))
        ...
        # 转为 numpy 数组并提取指定基因通道
        self.SR_ST_all = np.array(SR_ST_all).astype(np.float64)[:, gene_order]
        self.spot_ST_all = np.array(spot_ST_all).astype(np.float64)[:, gene_order]
        ...
        for data_arr in [self.SR_ST_all, self.spot_ST_all]:
            for i in range(data_arr.shape[0]):
                for j in range(data_arr.shape[1]):
                    if np.sum(data_arr[i, j]) != 0:
                        Min, Max = data_arr[i, j].min(), data_arr[i, j].max()
                        data_arr[i, j] = (data_arr[i, j] - Min) / (Max - Min + 1e-8)

        # 提取 gene 名称 BERT 向量
        self.tokenizer = AutoTokenizer.from_pretrained("/home/cbtil-3/QNF/bert", local_files_only=True)
        self.model = AutoModel.from_pretrained("/home/cbtil-3/QNF/bert", local_files_only=True)
        self.model.eval()
...
class Xenium5k2(Dataset):
    def __init__(self, data_root, dataset_use,SR_times, status, gene_num, all_gene, gene_order,gene_name_order):
        ...
        if dataset_use == 'VisiumHD_moueembryo_sorted_data1':
            sample_name = ['20240611mouseembryo']
        else:
            raise ValueError('Invalid dataset_use')
        ...
        # print('self.spot_ST_all',self.spot_ST_all.shape)
        # 加载 BERT 模型，提取基因名称特征
        self.tokenizer = AutoTokenizer.from_pretrained("/home/cbtil-3/QNF/bert",
                                                         local_files_only=True, trust_remote_code=True)
        self.model = AutoModel.from_pretrained("/home/cbtil-3/QNF/bert",
                                                 local_files_only=True, trust_remote_code=True)
        self.model.eval()

...
def load_data(data_root,dataset_use,status,SR_times,gene_num,all_gene,gene_order=None,gene_name_order=None
    ...
    if dataset_use=='Xenium':
       ...
       dataset= Xenium5k3(data_root,dataset_use,SR_times,status,gene_num,all_gene,gene_order,gene_name_order)
...
class Xenium5k3(Dataset):
    def __init__(self, data_root, dataset_use,SR_times, status, gene_num, all_gene, gene_order,gene_name_order):
        ...
        # print('self.spot_ST_all',self.spot_ST_all.shape)
        # 加载 BERT 模型，提取基因名称特征
        self.tokenizer = AutoTokenizer.from_pretrained("/home/cbtil3/xiaofei/ST_proj/NMI/bert",
                                                         local_files_only=True, trust_remote_code=True)
        self.model = AutoModel.from_pretrained("/home/cbtil3/xiaofei/ST_proj/NMI/bert",
                                                 local_files_only=True, trust_remote_code=True)
        self.model.eval()

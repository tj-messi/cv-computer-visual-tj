...
def load_data(data_root,dataset_use,status,SR_times,gene_num,all_gene,gene_order=None,gene_name_order=None
    ...
    if dataset_use=='Xenium':
        ...
        dataset= Xenium5k2_mouseembryo(data_root,dataset_use,SR_times,status,gene_num,all_gene,gene_order,gene_name_order)
    ...
    elif dataset_use=='Visium_CytAssist_human_Tonsil':
    ...
    return dataset
class Only_HE_E15 (Dataset):
    def __init__(self, data_root,dataset_use, status, SR_times, gene_num, all_gene, gene_order,gene_name_order):
        ...
        he_root = '/home/cbtil2/ST_proj/NMI/data/Visium_CytAssist_human_Tonsil/WSI/extract/20230824'
        patient_ids =  os.listdir(he_root)
        print(patient_ids)
        for pid in patient_ids:  
            patient_dir = os.path.join(he_root, pid)  
            if not os.path.isdir(patient_dir):  
                continue  
    
   
            patch_dir = patient_dir  
    
            if self.status != 'Test':  
                continue  
               
            path_5120 = os.path.join(patch_dir, '5120_to256.npy')
            path_320 = os.path.join(patch_dir, '320_to16.npy')
        # if not (os.path.exists(path_5120) and os.path.exists(path_320)):
        #     continue
            self.selected_patches.append(patch_dir)
            self.wsi_5120_paths.append(path_5120)
            print(path_5120)
            print(path_320)
            self.wsi_320_paths.append(path_320)
            self.patch_ids.append(pid)
            
        # 构造共享全零 ST 模板（只构造一次）
        size = 256 if SR_times == 10 else 128
        self.SR_ST_template = np.zeros((200, size, size), dtype=np.float32)[self.gene_order]
        self.spot_ST_template = np.zeros((200, 26, 26), dtype=np.float32)[self.gene_order]
     
        print(f"共加载 patch 数量: {len(self.selected_patches)}")
        self.tokenizer = AutoTokenizer.from_pretrained("/home/cbtil2/ST_proj/NMI/bert",
                                                         local_files_only=True, trust_remote_code=True)
        self.model = AutoModel.from_pretrained("/home/cbtil2/ST_proj/NMI/bert",
                                                 local_files_only=True, trust_remote_code=True)
        self.model.eval()
    ...
    def __getitem__(self, idx):
        wsi_5120 = np.load(self.wsi_5120_paths[idx]).astype(np.float32)
        wsi_5120 = np.transpose(wsi_5120, (2, 0, 1))

        wsi_320 = np.load(self.wsi_320_paths[idx]).astype(np.float32)
        wsi_320 = np.transpose(wsi_320, (0, 3, 1, 2))

        gene_class = self.gene_order

        gene_maps = [
            np.ones((256, 256, 1)) * gray_value_of_gene(g, self.gene_order) / 255.0
            for g in gene_class
        ]
        gene_index_map = np.concatenate(gene_maps, axis=2).transpose(2, 0, 1)
        patch_id = self.patch_ids[idx]
        if isinstance(patch_id, list):
            patch_id = patch_id[0]
        return (
            self.SR_ST_template,
            self.spot_ST_template,
            wsi_5120,
            wsi_320,
            gene_class,
            gene_index_map,
...
class Xenium5kovaryovariancancer(Dataset):
    def __init__(self, data_root, dataset_use, SR_times, status, gene_num, all_gene, gene_order, gene_name_order):
        ...
        self.tokenizer = AutoTokenizer.from_pretrained("/home/zeiler/NMI/bert", local_files_only=True, trust_remote_code=True)
        self.model = AutoModel.from_pretrained("/home/zeiler/NMI/bert", local_files_only=True, trust_remote_code=True)
        self.model.eval()
        ...
        metadata_prompt = ("Provide spatial transcriptomics data from the Xenium5k platform for human species, with a cancer condition, and ovary tissue type.")
        with torch.no_grad():
            inputs = self.tokenizer(metadata_prompt, return_tensors="pt")
            outputs = self.model(**inputs)
            self.metadata_feature = outputs.pooler_output.squeeze(0)
...
class Xenium5kfrozenmousebrain(Dataset):
    def __init__(self, data_root, dataset_use, SR_times, status, gene_num, all_gene, gene_order, gene_name_order):
        ...
        self.tokenizer = AutoTokenizer.from_pretrained("/home/cbtil2/ST_proj/NMI/bert", local_files_only=True, trust_remote_code=True)
        self.model = AutoModel.from_pretrained("/home/cbtil2/ST_proj/NMI/bert", local_files_only=True, trust_remote_code=True)
        self.model.eval()
...
class Xenium5k7(Dataset):
    def __init__(self, data_root, dataset_use,SR_times, status, gene_num, all_gene, gene_order,gene_name_order):
        ...
        if dataset_use == 'Visium_CytAssist_human_Tonsil':
            sample_name = ['20230824']
        else:
            raise ValueError('Invalid dataset_use')
        ...
        # 统一加载逻辑（确保三个数据部分顺序一致）
        for sample_id, patch_id in self.selected_patches:
            spot_ST = sp_sparse.load_npz(os.path.join(data_root,dataset_use, 'spot_ST/extract/',sample_id, patch_id, 'spot_ST.npz')).toarray().reshape(26, 26, 18085)
            spot_ST = np.transpose(spot_ST, axes=(2, 0, 1))
            spot_ST_all.append(spot_ST)
            wsi_path = f"{data_root}{dataset_use}/WSI/extract/20230824/{patch_id}/5120_to256.npy"
            WSI_5120 = np.load(wsi_path)
            WSI_5120 = np.transpose(WSI_5120, axes=(2, 0, 1))
            WSI_5120_all.append(WSI_5120)
            wsi_320_path =  f"{data_root}{dataset_use}/WSI/extract/20230824/{patch_id}/320_to16.npy"
            #print(wsi_320_path)
            WSI_320 = np.load(wsi_320_path)
            WSI_320 = np.transpose(WSI_320, axes=(0, 3, 1, 2))
            WSI_320_all.append(WSI_320)
        ...
        metadata_prompt = ("Provide spatial transcriptomics data from the Visium cytAssist platform for human species, with a healthy condition, and Tonsil tissue type.")
        with torch.no_grad():
            inputs = self.tokenizer(metadata_prompt, return_tensors="pt")
            outputs = self.model(**inputs)
            self.metadata_feature = outputs.pooler_output.squeeze(0)
...
class Xenium5k_dataset(Dataset):
    def __init__(self, data_root, dataset_use, SR_times, status, gene_num, all_gene, gene_order, gene_name_order):
        ...
        metadata_prompt = ("Provide spatial transcriptomics data from the VisiumHD platform for mouse species, with a healthy condition, and Whole Brain tissue type.")
        with torch.no_grad():
            inputs = self.tokenizer(metadata_prompt, return_tensors="pt")
            outputs = self.model(**inputs)
            self.metadata_feature = outputs.pooler_output.squeeze(0)
...
class Xenium5k2_mouseembryo(Dataset):
    def __init__(self, data_root, dataset_use,SR_times, status, gene_num, all_gene, gene_order,gene_name_order):
        '''
            data_root: 数据根目录路径
            SR_times: 下采样倍数

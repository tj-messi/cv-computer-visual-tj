...
class Xenium5k_dataset(Dataset):
    def __init__(self, data_root,dataset_use, SR_times, status, gene_num, all_gene, gene_order, gene_name_order):
        ...
        for sample_id, patch_id in self.selected_patches:
            ...
            if SR_times == 10:
                hr_st_file = os.path.join(hr_st_dir, "HR_ST_256_1000.npz")
                SR_ST = np.load(hr_st_file)['arr_0']
                SR_ST = np.transpose(SR_ST, axes=(2, 0, 1))  # 转为 (channels, H, W)
            elif SR_times == 5:
                hr_st_file = os.path.join(hr_st_dir, "HR_ST_128_1000.npz")
                SR_ST = np.load(hr_st_file)['arr_0']
        ...
        spot_ST_all = []
        for sample_id, patch_id in self.selected_patches:
            spot_st_path = f"{data_root}/spot_ST/extract/{sample_id}/{patch_id}/spot_ST_1000.npz"

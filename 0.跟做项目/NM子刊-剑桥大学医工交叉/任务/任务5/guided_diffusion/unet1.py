...
class UNetModel(nn.Module):
    ...
    def __init__(
        ...
        co_expression = np.load(root + 'Visiumhdmousebrain4_8/gene_coexpre.npy')
        ...
        co_expression = np.load(root + 'Visiumhdmousebrain4_8/gene_coexpre.npy')
        co_expression_new =co_expression[0:self.gene_num*10,0:self.gene_num*10]
        ...
        self.conv_layer1 = nn.Sequential(
            conv_nd(dims, 188, ch, 3, padding=1),
            nn.SiLU()
        )
    ...
    def forward(self, x, timesteps,low_res, WSI_5120,WSI_320, gene_class, Gene_index_map,gene_name_features, metadata_feature):#new1.5:, gene_class, Gene_index_map
        """
...
########################################################################
        ...
        gene_processor = GeneNameFeatureProcessor(out_channels=20, spatial_size=64)
        meta_processor = MetadataFeatureProcessor(out_channels=20, spatial_size=64)
        batch_size = x.shape[0]  
        device = Final_merge.device 
        
        
        processed_gene = gene_processor(gene_name_features)    # 输出 shape: [batch, 20, 64, 64]
        processed_meta = meta_processor(metadata_feature, batch_size)  
        processed_gene = processed_gene.to(device)
        processed_meta = processed_meta.to(device)# 输出 shape: [batch, 20, 64, 64]
        #new1.5#t2
        ...
        Final_merge = th.cat([Final_merge, Gene_index_map1,processed_gene,processed_meta], dim=1)
...
class GeneMemoryNetwork(nn.Module):
    ...
    def forward(self, single_gene_data):
        ...
        adapted_memory = self.meta_updater.adapt_memory(global_memory, queries)  # 动态调整 [num_genes, gene_dim]

        # 4. 稀疏筛选 (在调整后的记忆库中选择最相关基因)
        ...
        combined_memory = torch.cat([local_memory, sparse_memory], dim=1)  # [batch, query_dim * 2, gene_dim]# 组合局部与稀疏记忆

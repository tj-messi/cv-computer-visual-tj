...
from utils_level_class import save_clf_checkpoint, adjust_learning_rate, calc_acc, calc_precision_recall_fscore
from utils_level_class import calc_classwise_acc 
from torch.utils.data import DataLoader

from dataloader_level_class import EndoVis18VQAGPTClassification, Pit24VQAClassification
from model_level_class import PitVQANet
...
# 层级分类-大类
major_labels = [
            'surgical_step',      # 0-14 (steps)
            'surgical_phase',     # 15-18 (phases)
            'surgical_instruments',  # 19-36 (instruments)
            'instrument_count',   # 37-39 (numbers of instruments)
            'position',           # 40-44 (positions)
            'operation_description'  # 45-58 (operation descriptions)
            ]
...
label_to_major = [
            ...
            # Number of Instruments (37-39)
            3,3,3,
            # Positions (40-44)
            4,4,4,4,4,
            # Operations Descriptions (45-58)
...
def train(train_dataloader, model, criterion, optimizer, epoch, device):
    ...
    for i, (_, images, questions, labels) in enumerate(train_dataloader, 0):
        ...
        # 2025-2-14 zjz
        # classes
        class_labels = [label_to_major[label.item()] for label in labels]
        class_labels = torch.tensor(class_labels,dtype=torch.int64).to(device)
        

        class_outputs = torch.zeros((outputs.size(0), 6),dtype=torch.float32).to(device)
        for i in range(6):  # 6 major categories
            category_indices = [index for index, category in enumerate(label_to_major) if category == i]
            class_outputs[:, i] = outputs[:, category_indices].sum(dim=1)
        class_loss = criterion(class_labels,class_outputs)

...
class Pit24VQAClassification(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        left_step = 1
        right_step = 1
        while(len(raw_video)<4):
            right_idx = selected_idx + right_step
            left_idx = selected_idx - left_step
            ...
            if not os.path.exists(left_frame_path) :
            ...
            if not os.path.exists(right_frame_path) :
                ...
                if compare_images(img_loc,right_frame_path) > compare_images(img_loc,left_frame_path) :# 右边更像
                    right_frame = Image.open(right_frame_path).convert('RGB')
                    raw_video.append(right_frame)
                    right_step+=1
                    continue 

...
class VideoFeatureExtractor(nn.Module):
    ...
    def forward(self, video_frames):
        # Process video frames (assuming video_frames is a tensor of shape [num_frames, batch_size , channels, height, width])
        output = self.visual_encoder(video_frames)  # This returns a ModelOutput
        video_features = output.last_hidden_state  # Extract the last hidden state
        return video_features.mean(dim=1)  # Average over frames to get a single feature vector per video
...
class PitVQANet(nn.Module):
    ...
    def forward(self, image, video, question):
        ...
        video = video.permute(1, 0, 2, 3, 4) # VIT 接受 (batch_size,num_frames,channel,width,hight)
        ...
        # combine image-video
        combined_embeds = torch.cat(image_embeds.mean(1),video_embeds)

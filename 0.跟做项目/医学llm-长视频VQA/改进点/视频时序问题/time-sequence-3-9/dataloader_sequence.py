...
class Pit24VQAClassification(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        video_path = img_loc = os.path.join(seq_path, 'images', video_num)
        ...
        seletced_index = idx_list.index(selected_idx)

        left_step = skip
        right_step = skip
        while(len(raw_video)<8):
            raw_video.append(raw_image)
            '''while(seletced_index - left_step >=0):
                left_idx = idx_list[seletced_index - left_step]
                left_format_num = f"{left_idx:05d}"
                left_frame_path = os.path.join(seq_path, 'images', video_num, left_format_num + '.png')
                left_frame = Image.open(left_frame_path).convert('RGB')
                raw_video.append(left_frame)
                left_step+=skip
            while(seletced_index + right_step < video_len):
                right_idx = idx_list[seletced_index + right_step]
                right_format_num = f"{right_idx:05d}"
                right_frame_path = os.path.join(seq_path, 'images', video_num, right_format_num + '.png')
        ...
        # print(raw_video)
        # 最后根据95分位进行裁剪和填充
        raw_video = padding_and_slicing(raw_video,8)

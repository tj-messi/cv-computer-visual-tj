            ...
            for i in range(opts.test_batch_size):
                ...
                if opts.couple_outputs or global_i % 100 == 0:
                    ...
                    if opts.resize_factors is not None:
                        ...
                        print(np.array(source.resize(resize_amount)).shape)
                        #print(np.array(input_im.resize(resize_amount, resample=Image.NEAREST)).shape)
                        #print(np.array(result.resize(resize_amount)).shape)
...
#    print("cur_latent_codes", cur_latent_codes)
    ...
    for pic_name in history_latent_codes:
        calc_sim = torch.nn.CosineSimilarity(dim=0, eps=1e-06)
        similarity = calc_sim(torch.flatten(cur_latent_codes[0][:feature_dim]),torch.flatten(history_latent_codes[pic_name][:feature_dim]))
        #similarity = calc_sim(torch.flatten(cur_latent_codes[0][18-feature_dim:]),torch.flatten(history_latent_codes[pic_name][18-feature_dim:]))
#        print("similarity:", similarity)
    ...
    if False or highest_similarity < shredhold or highest_similarity == -999:
        wanted_pic_name = ''
        wanted_latent_codes = cur_latent_codes
#    print(wanted_pic_name)
#    print(wanted_latent_codes)

...
###np.random.choice
用于从给定的一维数组中随机抽取元素。你可以指定抽取的元素数量，以及是否允许重复抽取（即有放回抽样）或不允许重复抽取（即无放回抽样）

	numpy.random.choice(a, size=None, replace=True, p=None)
**a**：一维数组，表示候选元素集合。

**size**：int 或 tuple of ints，可选。表示要抽取的元素数量。如果为 int，则抽取指定数量的元素；如果为 tuple，则抽取指定形状的元素数组。默认值为 None，即抽取一个元素。

**replace**：boolean，可选。表示是否允许重复抽取。如果为 True（默认值），则允许重复抽取；如果为 False，则不允许重复抽取（即无放回抽样）。
